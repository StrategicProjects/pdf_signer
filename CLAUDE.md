...
- Keys: RSA, ECDSA (P-256/P-384), Ed25519. Revocation: **authenticated** CRL +
  OCSP (signature/freshness/scope), **soft-fail** on unknown.
- **RFC 5280 path validation** (name constraints + certificate-policy engine,
  path-building **backtracking**) **validated against the NIST PKITS suite**
  (42/42 policy + 38/38 name-constraint).
- Verification locates signatures **structurally** (not byte scanning),
  validates RFC 3161 timestamp tokens cryptographically, and judges the chain at
  a trusted timestamp's `genTime` for B-T+.
...
- **Python**: `pdfsignerpy` (PyO3) — depends on this crate as a **git dep pinned
  to a tag** (currently `v0.2.0`). Bump the tag there after a release here.

## Release
Tag `vX.Y.Z` + GitHub release. Current: **v0.2.0**. Bump `version` in `Cargo.toml`.
